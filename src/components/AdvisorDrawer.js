...
export default function AdvisorDrawer(props) {
  let a;
  let b;
  let c;
  let d;
  ...
  let{manifest,setManifest,setAddManifestDisplay,setManifest_display,setEditManifestBoolean,addAdmin,setAddAdmin,reviewerTimeSlotDisplay,setReviewerTimeSlotDisplay}=useContext(PostContext)
  ...
  return (
    <div style={{backgroundColor:"gray",height: "100vh",width:"2vw"}}>
        <div className='pt-5'><button onClick={()=>{navigate('/advisor_home');setClose(!(close))}}>{a?<ChatIcon sx={{color:"black"}}/>:<ChatIcon sx={{color:"white"}}/>}</button></div>
